...
jQuery( document ).ready(function() {
	...
	jQuery(window).on('scroll', function(e) {
		e.preventDefault();
        var latestLoadMoreElement = jQuery('#latestloadMore');
		if (latestLoadMoreElement.length > 0){
			/*if (jQuery(window).scrollTop() >= jQuery('#latestloadMore').offset().top + jQuery('#latestloadMore').outerHeight() - window.innerHeight)*/
            if (jQuery(window).scrollTop() >= latestLoadMoreElement.offset().top + latestLoadMoreElement.outerHeight() - window.innerHeight) 
             {
