...
function show_ti_post_by_resourceType( $tag_name ){
	...
	if ($loop->have_posts()) {
		...
		while ($loop->have_posts()) : $loop->the_post();
			...
			$out.='<div class="resource-card col-lg-4 col-md-6 col-sm-6 col-12 ">
				...
				<div class="td-module-container td-category-pos-above post-box">
					...
					<div class="td-module-meta-info123 content-box">';
						...
						$out.='<div class="td-post-client-name" style="float:right;margin:10px 0;"><a href="'.get_term_link($sponsored_by_single->term_id).'">'. $sponsored_by_single->name.'</a> </div>';
						}
			...
			if( $i % 3 == 0 ){
				$out.='<div class="clearfix visible-lg"></div>';
			}
			if( $i % 2 == 0 ){
				$out.='<div class="clearfix visible-sm visible-md"></div>';
			}
...
function show_ti_post_ajax(){
	...
	if ($loop->have_posts()) {
		...
		while ($loop->have_posts()) : $loop->the_post();
			...
			$out.='<div class="resource-card col-lg-4 col-md-4 col-sm-6 col-12 ">
				...
				<div class="td-module-container td-category-pos-above post-box">
					...
					<div class="td-module-meta-info123 content-box">';
						...
						*/
						...
						$out.='<a class="readMore" href="'.get_the_permalink().'" rel="bookmark" title="'.the_title_attribute("echo=0").'">Download </a>';
					$out.='</div>
			...
			if( $i % 3 == 0 ){
				$out.='<div class="clearfix visible-lg visible-md"></div>';
			}
			if( $i % 2 == 0 ){
...
function show_ti_posts_fn( $atts ){
	...
	<div class="heading_wrapper">
		...
		<div class="col-lg-8 col-md-8 col-sm-8 col-12 resources-types">
		...
		<span class="heading-category all">
			...
			foreach ($resource_types as $resource_type) {
				...
				<span class="heading-category">
					<a class="resource-type-link resource-type-<?php echo $resource_type->slug; ?>" data-type-id="<?php echo $resource_type->term_id; ?>">
					<span class="tdb-sibling-cat-bg"></span><?php if($resource_type->slug == 'case-study')
				{
...
function add_hide_URLstring_script() {
    if (is_post_type_archive('resources')) { // Check if it's the "Resources" archive page
        ?>
		<script>
		function hide_URLstring(){
...
add_action('wp_head', 'add_hide_URLstring_script');
?>