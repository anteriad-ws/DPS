...
class HRD_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'herald_posts_widget', 'description' => esc_html__( 'Display your posts with this widget', 'herald-buddy' ) );
		$control_ops = array( 'id_base' => 'herald_posts_widget' );
		parent::__construct( 'herald_posts_widget', esc_html__( 'Herald Posts', 'herald-buddy' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => esc_html__( 'Posts', 'herald-buddy' ),
			'style' => 'g1',
			'slider' => 0,
			'numposts' => 5,
			'category' => array(),
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $instance['manual'] ) && !empty( $instance['manual'][0] ) ) {
			...
			if ( $instance['orderby'] == 'views' && function_exists( 'herald_is_wp_post_views_active' ) && herald_is_wp_post_views_active() ) {
		...
		<div class="row <?php echo esc_attr( $slider_class ); ?>">
			...
			<?php while ( $herald_posts->have_posts() ) : $herald_posts->the_post(); ?>
				<?php include HERALD_BUDDY_DIR . 'inc/widgets/posts-templates/content-'.$instance['style'].'.php'; ?>
			<?php endwhile; ?>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title', 'herald-buddy' ); ?>:</label>
		...
		<p>
	   	 	<label for="<?php echo esc_attr($this->get_field_id( 'numposts' )); ?>"><?php esc_html_e( 'Number of posts to show', 'herald-buddy' ); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id( 'slider' )); ?>"><?php esc_html_e( 'Apply slider', 'herald-buddy' ); ?></label>
		</p>
	  	...
	  	<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id( 'cat_link' )); ?>"><?php esc_html_e( 'Display category link', 'herald-buddy' ); ?></label>
		</p>
	   ...
	   <p>
	   	 <label for="<?php echo esc_attr($this->get_field_id( 'manual' )); ?>"><?php esc_html_e( 'Or choose manually', 'herald-buddy' ); ?>:</label>
		 ...
		 <small class="howto"><?php esc_html_e( 'Specify post ids separated by comma if you want to select only those posts. i.e. 213,32,12,45 Note: you can also choose pages as well as custom post types', 'herald-buddy' ); ?></small>
	   </p>
	  ...
	  <p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_detect' )); ?>"><?php esc_html_e( 'Auto detect category', 'herald-buddy' ); ?></label>
		<small class="howto"><?php esc_html_e( 'If sidebar is used on single post template, display posts from current post category ', 'herald-buddy' ); ?></small>
	  </p>

	   <p>
	   	 <label for="<?php echo esc_attr($this->get_field_id( 'tag' )); ?>"><?php esc_html_e( 'Tagged with', 'herald-buddy' ); ?>:</label>
		 ...
		 <small class="howto"><?php esc_html_e( 'Specify one or more tags separated by comma. i.e. life, cooking, funny moments', 'herald-buddy' ); ?></small>
	   </p>
	...
	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'time' )); ?>"><?php esc_html_e( 'Only select posts which are not older than', 'herald-buddy' ); ?>:</label>
	...
	function widget_orderby( $widget_instance = false, $orderby = false ) {
		...
		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'orderby' )); ?>"><?php esc_html_e( 'Order by:', 'herald-buddy' ); ?></label>
	...
	function widget_format( $widget_instance = false, $format = false ) {
		...
		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'format' )); ?>"><?php esc_html_e( 'Format:', 'herald-buddy' ); ?></label>
...
?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'category' )); ?>"><?php esc_html_e( 'Choose from:', 'herald-buddy' ); ?></label><br/>
	...
	function widget_style( $widget_instance = false, $current = false ) {
		...
		$styles = array(
			'g1' => esc_html__( 'List', 'herald-buddy' ),
			'f' => esc_html__( 'Big', 'herald-buddy' ),
			'f1' => esc_html__( 'Big (overlay)', 'herald-buddy' )
		);

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'style' )); ?>"><?php esc_html_e( 'Widget style:', 'herald-buddy' ); ?></label>
	...
	function widget_meta( $widget_instance = false, $current = false ) {
		...
		if ( !empty( $widget_instance ) ) : ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'meta' )); ?>"><?php esc_html_e( 'Display meta data:', 'herald-buddy' ); ?></label><br/>
